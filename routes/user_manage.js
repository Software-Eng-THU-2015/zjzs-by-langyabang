...
router.get("/vote", function(req, res){
	var acts = [];
	db[VOTE_DB].find({},function(err, docs){
		if (err) {
			res.send("数据库查找投票活动列表出错，请检查数据库。");
			return;
		}
		for (var i = 0; i < docs.length; i++) {
			var activity = {
				...
				start_time: docs[i].start_time,
				end_time: docs[i].end_time,
				id: docs[i]["_id"].toString()
			};
			acts[i] = activity;
		}
		res.render("activity_vote", {
			acts: acts
		});
	});
});

router.get("/addvote", function(req, res) {
	var activity = {name: "新建活动"};
	res.render("add_vote", {activity: activity});
});
...
router.post("/addvote", function(req, res) {
	...
	// var start_time = req.body.inputstartyear+ req.body.inputstartmonth+ req.body.inputstartday+ req.body.inputstarthour+ req.body.inputstartminute;
	// var end_time = req.body.inputendyear+ req.body.inputendmonth+ req.body.inputendday+ req.body.inputendhour+ req.body.inputendminute;
	var start_time = new Date(req.body.inputstartyear, req.body.inputstartmonth, req.body.inputstartday, req.body.inputstarthour, req.body.inputstartminute).valueOf();
	var end_time = new Date(req.body.inputendyear, req.body.inputendmonth, req.body.inputendday, req.body.inputendhour, req.body.inputendminute).valueOf();
	start_time = parseInt(start_time);
	end_time = parseInt(end_time);
	var questions_number = req.body.questions_number;

	var question1 = req.body.question1;
	var question2 = req.body.question2;

	var poll_number = questions_number;
	var poll_question = [];
	poll_question.push(question1);
	poll_question.push(question2);
	var option = new Array(2);
	option[0] = new Array(4);
	option[1] = new Array(4);
	option[0][0] = (req.body.option11);
	option[0][1] = (req.body.option12);
	option[0][2] = (req.body.option13);
	option[0][3] = (req.body.option14);
	option[1][0] = (req.body.option21);
	option[1][1] = (req.body.option22);
	option[1][2] = (req.body.option23);
	option[1][3] = (req.body.option24);
	var options_tickets = new Array(2);
	for (var i = 0; i < 2; i++){
		options_tickets[i] = new Array(4);
		for (var j = 0 ; j < 4; j++)
			options_tickets[i][j] = 0;
	}
	...
	var act = {
		...
		'end_time': end_time,
		'quesions_num': poll_number,
		'questions': poll_question,
		'options': option,
		'options_num':[4,4],
		'options_min':[1,1],
		'options_max':[1,4],
		'options_tickets':options_tickets
	};
	db[VOTE_DB].insert(act, function(){
		res.send('insert ok.');
	});
});
