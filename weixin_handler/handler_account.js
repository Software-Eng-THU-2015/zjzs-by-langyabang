...
function sendBindInfo(msg,res,openID)//发送绑定返回的内容
{
    res.send(
...
//======================================
exports.check_votable_activity=function(msg)
{
    if (msg.MsgType[0]==="text")
        if (msg.Content[0]==="投啥")
            return true;
...
exports.faire_votable_activity=function(msg,res)
{
    act_infoer.getCurrentActivity_VOTE(function(docs)
    {
        if (docs.length===0)
        {
            res.send(template.getPlainTextTemplate(msg,"对不起，最近没有可以投票的活动 orz..."));
            return;
        }
        var openID=msg.FromUserName[0];
        console.log(openID);
        ...
        for (var i=0;i<docs.length;i++)
        {
            ...
            tmpEle[template.rich_attr.url]=urls.voteActivity+"?actid="+docs[i].id+"&userid="+openID;
            tmpEle[template.rich_attr.picture]=docs[i].pic_url;
            showList.push(tmpEle);
        }
        res.send(template.getRichTextTemplate(msg,showList));
